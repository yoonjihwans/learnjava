...
<%
    ...
    int totalProducts = productDAO.getTotalProductsByType1(filter);
    ...
    List<ProductDTO> products = productDAO.selectProductListByType1WithPaging(startRow, endRow, filter);
%>
<main>
    <section class="product-list">
        <h2>Camera</h2>
        <div class="filter">
            <a href="<%=request.getContextPath()%>/index.jsp?workgroup=product&work=camera_list&filter=new">신상품</a> | 
            <a href="<%=request.getContextPath()%>/index.jsp?workgroup=product&work=camera_list&filter=lowestPrice">낮은가격</a> | 
            <a href="<%=request.getContextPath()%>/index.jsp?workgroup=product&work=camera_list&filter=highestPrice">높은가격</a>
        </div>
        <div class="products">
        ...
        <div class="pagination">
            <% if (pageNum > 1) { %>
                <a href="<%=request.getContextPath()%>/index.jsp?workgroup=product&work=camera_list&pageNum=<%= pageNum - 1 %>&filter=<%= filter %>">&laquo;</a>
            <% } %>
            <% for (int i = 1; i <= totalPage; i++) { %>
                <a href="<%=request.getContextPath()%>/index.jsp?workgroup=product&work=camera_list&pageNum=<%= i %>&filter=<%= filter %>" class="<%= (i == pageNum) ? "active" : "" %>"><%= i %></a>
            <% } %>
            <% if (pageNum < totalPage) { %>
                <a href="<%=request.getContextPath()%>/index.jsp?workgroup=product&work=camera_list&pageNum=<%= pageNum + 1 %>&filter=<%= filter %>">&raquo;</a>
            <% } %>
        </div>
        <div class="floating">
